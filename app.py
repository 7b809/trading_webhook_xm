...
from flask import Flask, request, jsonify, render_template
from config import Config, WEBHOOK_MAP
...
# ✅ NEW: Webhook → Collection Mapping (kept same)
WEBHOOK_MAP = {
    1: "nifty",
    2: "banknifty",
    3: "crypto"
...
def webhook_dynamic(webhook_id):
    ...
    collection_name = WEBHOOK_MAP.get(webhook_id)
...
if __name__ == "__main__":
    app.run(port=Config.PORT, debug=True, use_reloader=False)from db import alerts_collection, db
from telegram import send_telegram_message
from parser import parse_alert
import pytz
from logger import logger
